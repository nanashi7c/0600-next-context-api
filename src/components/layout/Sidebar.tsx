          ...
          <ul>
            ...
            <ul className="mt-4 mb-4 ml-8 text-(--font-color-dark) ">
              {projects?.data.map((project) => (
                <li
                  key={project.slug}
                  className={`${isActive(`/projects/${project.slug}`) ? "bg-(--hover-color) font-bold" : ""} cursor-pointer block pt-4 pr-4 pb-4 pl-8 whitespace-nowrap text-sm hover:bg-[#8fe3c7]/25`}
                >
                  {/* プロジェクト名・期限の横並べ */}
                  <Link
